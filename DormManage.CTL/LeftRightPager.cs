...
namespace ExtendGridView
{
    ...
    /// 自定义分页控件
    /// </summary>
    [ToolboxData("<{0}:LeftRightPager runat=\"server\"></{0}:LeftRightPager>")]
    public class LeftRightPager : WebControl, IPostBackEventHandler, INamingContainer
    {
        ...
        {
            string templateCell = "<td ><a onclick=\"{0}\">" + lastEnable + "</a></td>";
        ...
        {
            string templateCell = "<td><a onclick=\"{0}\">" + previousEnable + "</a></td>";
        ...
        {
            string templateCell = "<td><a onclick=\"{0}\"> " + nextEnable + "</a></td>";
            return String.Format(templateCell, Page.ClientScript.GetPostBackClientHyperlink(this, (CurrentIndex + 1).ToString()));
        }
        //下一页不可用时的文本
